...
extension FileManager {
    func getFilePathsWithPrefix(baseFolder: String, prefix: String) -> [String] {
        do {
            let fileURLs = try self.contentsOfDirectory(at: URL(fileURLWithPath: baseFolder), includingPropertiesForKeys: nil)
            return fileURLs.filter { $0.lastPathComponent.hasPrefix(prefix) }.map { $0.path }
        } catch {
            print("Error while enumerating files \(baseFolder): \(error.localizedDescription)")
            return []
        }
    }
}
