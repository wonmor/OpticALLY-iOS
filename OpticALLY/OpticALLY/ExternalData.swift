...
struct ExternalData {
    ...
    static func createOBJString(for node: SCNNode) -> String {
           guard let geometry = node.geometry,
                 let vertexSource = geometry.sources.first(where: { $0.semantic == .vertex }) else {
               print("Node does not have valid geometry")
               return ""
           }
           ...
           var objString = "# OBJ file generated by OpticALLY\n"
           ...
           for element in geometry.elements {
               ...
               case .triangles:
                   let indicesCount = data.count / bytesPerIndex
                   for i in 0..<indicesCount where i % 3 == 0 {
                       let indices = (0..<3).map { j -> Int in
                           let indexOffset = (i + j) * bytesPerIndex
                           return data.index(at: indexOffset, bytesPerIndex: bytesPerIndex)
                       }
                       objString += "f \(indices[0]+1) \(indices[1]+1) \(indices[2]+1)\n"
                   }
               default:
